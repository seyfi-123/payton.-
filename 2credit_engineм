...
class Config:
    ...
    MAX_REQUEST_SIZE = 1_048_576  # 1 MB — ИСЛОҲ №4
    ...
    REQUIRED_ENV = (
        'DB_PASSWORD', 'API_KEY', 'SIGNING_SECRET', 'CIB_URL', 'CIB_API_KEY',
    ...
    def validate(cls):
        ...
        if cls.ENV == 'production':
            ...
            if len(signing_secret) < 32:
                raise ValueError('SIGNING_SECRET >= 32 chars')
...
class CreditEngine:
    ...
    async def _stage1_create_pending(self, passport_hmac, product_type, amount, request_id):
...
# ============================================================================
# ИСЛОҲ №4: Middleware / FIX #4: Middleware
# ============================================================================
...
# ============================================================================
# SIGNATURE MIDDLEWARE / X-Signature + replay protection
# Canonical form:
# METHOD\nPATH\nTIMESTAMP\nSHA256(BODY)
# Header: X-Signature: TIMESTAMP.HEX_HMAC
# ============================================================================
SIGNATURE_TTL_SEC = int(os.environ.get('SIGNATURE_TTL_SEC', '300'))
_SIGNATURE_REPLAY = {}


def _signature_error(status_code, code, reason):
    return StarletteJSON(
        status_code=status_code,
        content={'code': code, 'status': 'ERROR', 'reason': reason})


@app.middleware("http")
async def signature_middleware(request, call_next):
    # GET/HEAD/OPTIONS endpoints (health/read-only) are not signed.
    if request.method not in ('POST', 'PUT', 'PATCH', 'DELETE'):
        return await call_next(request)

    secret = os.environ.get('SIGNING_SECRET', '')
    if not secret:
        return _signature_error(500, 'E2004', 'SIGNING_SECRET not configured.')

    header = request.headers.get('X-Signature')
    if not header:
        return _signature_error(401, 'E1022', 'Missing X-Signature.')

    try:
        ts_text, supplied_sig = header.split('.', 1)
        timestamp = int(ts_text)
        if not supplied_sig or not re.fullmatch(r'[0-9a-fA-F]{64}', supplied_sig):
            raise ValueError('bad signature format')
    except (ValueError, TypeError):
        return _signature_error(401, 'E1022', 'Invalid X-Signature format.')

    now = int(time.time())
    if abs(now - timestamp) > SIGNATURE_TTL_SEC:
        return _signature_error(401, 'E1022', 'X-Signature timestamp expired.')
    ...
    if not hmac.compare_digest(supplied_sig.lower(), expected_sig.lower()):
        return _signature_error(401, 'E1022', 'Invalid X-Signature.')

    # Replay protection. Keep only recent entries; this is per process.
    replay_key = f'{timestamp}.{supplied_sig.lower()}'
    cutoff = now - SIGNATURE_TTL_SEC
    for key, seen_at in list(_SIGNATURE_REPLAY.items()):
        if seen_at < cutoff:
            _SIGNATURE_REPLAY.pop(key, None)
    if replay_key in _SIGNATURE_REPLAY:
        return _signature_error(409, 'E1023', 'Request signature already used.')
    _SIGNATURE_REPLAY[replay_key] = now
...
# ============================================================================
# ИСЛОҲ №1: API Key холӣ рад / FIX #1: Reject empty API key
# ============================================================================
...
# ============================================================================
# ИСЛОҲ №2: Валидаторҳо / FIX #2: Validators
# ============================================================================
