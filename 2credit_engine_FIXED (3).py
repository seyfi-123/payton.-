...
class Config:
    ...
    # Патчи амниятӣ #3: fallback барои нокомии Face-ID/CIB (PENDING_REVIEW)
    REVIEW_ESCALATION_INTERVAL_SEC = 1800    # ҳар 30 дақиқа тафтиш
    REVIEW_MAX_WAIT_HOURS = 24               # аз ин зиёд — REJECTED автомат
...
class CreditEngine:
    def __init__(self, db_pool, crypto, cib_client, face_client,
        ...
        self._leader_cleanup = LeaderElection(db_pool, 'cleanup', 4,
                                               Config.LEADER_ELECTION_ENABLED)
        self._leader_review = LeaderElection(db_pool, 'review_escalation', 5,
                                              Config.LEADER_ELECTION_ENABLED)
    ...
    async def process_loan_application(self, passport_sn, face_id_data,
        ...
        if stage2['status'] != 'OK':
            ...
            if user_phone:
                msg = ('Аризаи шумо барои кредит қабул шуд. Дар айни '
                       'ҳол системаи тасдиқ муваққатан дастнорас аст — '
                       'мо аризаи шуморо нигоҳ доштем ва ба зудӣ бо '
                       'шумо тамос мегирем.')
                task = asyncio.create_task(
                    self._deliver_otp_sms(user_phone, msg, None))
                self._bg_tasks.add(task)
                task.add_done_callback(self._bg_tasks.discard)
            return {
                'code': 'E0003', 'status': 'PENDING_REVIEW',
                'application_id': application_id,
                'message': 'Аризаи шумо қабул шуд ва дар арафаи баррасии '
                            'иловагист. Мо ба зудӣ бо шумо тамос мегирем.',
                'http_status': 200,
            }
        ...
        async with self.db.transaction() as conn:
            ...
            if existing:
                ...
                if existing['status'] == 'PENDING_REVIEW':
                    # PENDING_REVIEW ҳолати БАРҚАРОР аст (мизоҷ аллакай
                    # ҷавоби 200-и хушмуомила гирифтааст) — на ҳолати
                    # "нокоми, лозим такрор". Такрори ҳамон request_id
                    # бояд ҳамон ҷавоби 200-ро дуруст такрор кунад.
                    return {
                        'code': 'E0003', 'status': 'PENDING_REVIEW',
                        'application_id': existing['id'],
                        'message': 'Аризаи шумо қабул шуд ва дар арафаи '
                                    'баррасии иловагист. Мо ба зудӣ бо '
                                    'шумо тамос мегирем.',
                        'http_status': 200,
                    }
    ...
    async def _review_escalation_loop(self):
        """Патчи амниятӣ #3: агар ariza аз сабаби нокомии Face-ID/CIB
        дар PENDING_REVIEW бештар аз REVIEW_MAX_WAIT_HOURS монад — ба
        ҷои он ки абадан дар ҳаво овезон бимонад, ба таври возеҳ
        REJECTED мешавад ва мизоҷ бо СМС хабардор мешавад."""
        while not self._shutdown_event.is_set():
            try:
                if await self._leader_review.try_acquire():
                    await self._escalate_stale_reviews()
            except Exception as e:
                logger.exception(f'Review escalation loop: {scrub_text(str(e))}')
            try:
                await asyncio.wait_for(
                    self._shutdown_event.wait(),
                    timeout=Config.REVIEW_ESCALATION_INTERVAL_SEC)
                return
            except asyncio.TimeoutError:
                continue
    ...
    async def _escalate_stale_reviews(self):
        async with self.db.transaction() as conn:
            rows = await conn.fetch("""
                SELECT id, passport_sn_hmac FROM applications
                WHERE status = 'PENDING_REVIEW'
                  AND updated_at < NOW() - ($1 * INTERVAL '1 hour')
                FOR UPDATE SKIP LOCKED
            """, Config.REVIEW_MAX_WAIT_HOURS)
            for r in rows:
                await self._update_status(
                    conn, r['id'], 'REJECTED',
                    f'Хидматҳои беруна (Face-ID/CIB) зиёда аз '
                    f'{Config.REVIEW_MAX_WAIT_HOURS} соат дастнорас монданд — '
                    f'ariza ба таври худкор баста шуд.')
                await self._cancel_pending_loan(
                    conn, r['id'], 'review escalation timeout')
                await self._audit(conn, 'REVIEW_ESCALATED_REJECTED', r['id'],
                                  r['passport_sn_hmac'],
                                  f'PENDING_REVIEW > {Config.REVIEW_MAX_WAIT_HOURS}h')
                logger.warning(
                    f'app_id={r["id"]} REJECTED пас аз '
                    f'{Config.REVIEW_MAX_WAIT_HOURS}с дар PENDING_REVIEW. '
                    f'ЭЗОҲ: applications ҷадвал рақами телефон нигоҳ '
                    f'намедорад (то stage4) — СМС-и ниҳоӣ фиристода '
                    f'НАМЕШАВАД. Бонк бояд ин ҳолатро аз audit_log '
                    f'пайгирӣ кунад ё дастӣ бо мизоҷ тамос гирад.')
    ...
    def start_background_workers(self):
        ...
        if self._review_task is None:
            self._review_task = asyncio.create_task(self._review_escalation_loop())

    async def stop_background_workers(self):
        self._shutdown_event.set()
        for t in (self._auto_debit_task, self._ttl_task,
                  self._overdue_task, self._cleanup_task, self._review_task):
            if t:
                try:
    ...
    async def health_check(self):
        ...
        checks['review_escalation_worker'] = (
            'RUNNING' if self._review_task and not self._review_task.done()
            else 'STOPPED')
