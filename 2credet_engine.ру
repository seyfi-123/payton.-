...
class CIBClient(BaseAsyncClient):
    ...
    async def _do_check(self, passport_sn: str) -> Dict:
        ...
        if os.environ.get('MOCK_MODE', '').lower() == 'true':
            logger.info('[MOCK] CIB check bypassed')
            return {'score': 700, 'limit': '50000'}
...
class FaceIDClient(BaseAsyncClient):
    ...
    async def _do_verify(self, face_image: str, passport_sn: str) -> Dict:
        # ★★★ ТАҒЙИРИ 2: MOCK_MODE ★★★
        if os.environ.get('MOCK_MODE', '').lower() == 'true':
            logger.info('[MOCK] Face-ID bypassed')
...
class ABSClient(BaseAsyncClient):
    ...
    async def _do_create(self, payload: Dict, idem: str) -> Dict:
        # ★★★ ТАҒЙИРИ 3: MOCK_MODE ★★★
        if os.environ.get('MOCK_MODE', '').lower() == 'true':
            logger.info('[MOCK] ABS bypassed')
            return {'contract_id': f'ABS-MOCK-{idem[:12]}'}
        if not idem:
...
class PaymentGatewayClient(BaseAsyncClient):
    ...
    async def _do_debit(self, payload: Dict, idem: str) -> Dict:
        # ★★★ ТАҒЙИРИ 4: MOCK_MODE ★★★
        if os.environ.get('MOCK_MODE', '').lower() == 'true':
            logger.info('[MOCK] Payment bypassed')
            return {'success': True, 'transaction_id': f'PAY-MOCK-{idem[:12]}'}
        if not idem:
            raise ValueError('idempotency_key required')
...
class SMSClient(BaseAsyncClient):
    ...
    async def send(self, phone: str, message: str) -> bool:
        # ★★★ ТАҒЙИРИ 5: MOCK_MODE ★★★
        if os.environ.get('MOCK_MODE', '').lower() == 'true':
            logger.info(f'[MOCK SMS] To: {phone}, Msg: {message}')
            return True
        try:
