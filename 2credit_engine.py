...
async def lifespan(app: FastAPI):
    global _executor_ref, _application_ref

    # ------------------------------------------------------------------
    # ENGINE INITIALIZATION
    # ------------------------------------------------------------------
    # ХАТО ТУЗАТИЛДИ: Илгари бу инициализация фақат Application.run() ичида
    # (ya'ni faqat "python credit_engine.py" bilan to'g'ridan-to'g'ri ishga
    # tushirilganda) bajarilar edi. Lekin Dockerfile'da buyruq
    #   uvicorn credit_engine:app --host 0.0.0.0 --port 8000
    # bo'lgani uchun modul faqat IMPORT qilinadi, __name__ != '__main__',
    # va Application() hech qachon yaratilmas, set_engine() chaqirilmas edi.
    # Natijada _engine doim None qolib, /system/ready va boshqa endpointlar
    # "Engine not initialized" (500) xatosini qaytarardi.
    #
    # Endi butun initialization FastAPI lifespan ichiga ko'chirildi — bu esa
    # uvicorn ishga tushirilishidan qat'i nazar (to'g'ridan-to'g'ri skript
    # sifatida ham, "uvicorn module:app" orqali ham) HAR DOIM chaqiriladi.
    ...
    try:
        ...
        logger.critical(
            f'FATAL: Application failed to start: {scrub_pii(str(e))}')
        raise
    ...
    if application.engine:
        application.engine.start_background_workers()

    logger.info('Lifespan startup complete — engine is ready')
...
class Application:
    ...
    def run(self):
        # ЭСЛАТМА: self.start() энди бу ерда чақирилмайди — уни FastAPI
        # lifespan (yuqorida) ishga tushiradi. Agar shu yerda ham chaqirsak,
        # engine, DB pool va bank klientlari IKKI marta yaratilib qoladi.
...
if __name__ == '__main__':
    # Bu blok faqat "python credit_engine.py" bilan to'g'ridan-to'g'ri
    # ishga tushirilganda bajariladi (mahalliy/qo'lda test uchun).
    # Production'da (Dockerfile'dagi) "uvicorn credit_engine:app ..."
    # buyrug'i bu blokni chetlab o'tadi — shuning uchun barcha real
    # initialization FastAPI lifespan ichida bo'lishi SHART.
    Application().run()